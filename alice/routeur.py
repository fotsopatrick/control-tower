...
# Choix du cerveau à l'exécution (bascule possible depuis l'interface web) :
#  - None / "auto" : on suit l'environnement (ALICE_BRAIN_AZURE=1 -> Azure, sinon Qwen local) ;
#  - "azure"       : force le cerveau Azure OpenAI (gpt-5-mini) ;
#  - "qwen"        : force le cerveau Qwen local.
_MODELE_FORCE = None


def choisir_modele(nom):
    """Fixe le cerveau utilisé par call_model. nom: 'auto' | 'azure' | 'qwen'."""
    global _MODELE_FORCE
    if nom is None or str(nom).strip().lower() in ("auto", "env", ""):
        _MODELE_FORCE = None
    else:
        _MODELE_FORCE = str(nom).strip().lower()
    return modele_actuel()


def modele_actuel():
    """Renvoie le nom du cerveau effectivement actif ('qwen' | 'azure')."""
    if _MODELE_FORCE == "azure":
        return "azure"
    if _MODELE_FORCE == "qwen":
        return "qwen"
    return "azure" if os.environ.get("ALICE_BRAIN_AZURE") == "1" else "qwen"


def force_modele():
    """True si le cerveau a été forcé manuellement (sinon False = env)."""
    return _MODELE_FORCE is not None
...
def call_model(messages, temperature=0.2, max_tokens=256):
    ...
    Cerveaux possibles (par défaut selon l'environnement, basculable en direct
    via choisir_modele) :
    ...
    env_azure = os.environ.get("ALICE_BRAIN_AZURE") == "1"
    if _MODELE_FORCE == "azure":
        azure = True
    elif _MODELE_FORCE == "qwen":
        azure = False
    else:
        azure = env_azure
