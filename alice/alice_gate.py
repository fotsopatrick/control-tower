...
from routeur import Routeur, call_model, choisir_modele, modele_actuel, force_modele
...
@app.route("/api/v1/modele", methods=["GET", "POST"])
def modele():
    """Lire (GET) ou basculer (POST) le cerveau d'Alice.

    GET  -> {modele: "qwen" | "azure", force: bool}
    POST {modele: "auto" | "qwen" | "azure"} -> choix appliqué + actif
    """
    if request.method == "POST":
        data = request.get_json(force=True, silent=True) or {}
        nom = data.get("modele") or data.get("cerveau") or "auto"
        if str(nom).lower() not in ("auto", "qwen", "azure"):
            return jsonify({"error": "modele inconnu (attendu: auto | qwen | azure)"}), 400
        actif = choisir_modele(nom)
        return jsonify({"ok": True, "choix": nom,
                        "modele": actif,
                        "force": nom != "auto"}), 200
    actif = modele_actuel()
    return jsonify({"modele": actif, "force": force_modele()}), 200
